...
class EnergyStorageEnv(gym.Env):
  ...
  def __init__(self, start_date: str) -> None:
  	...
  	self.cur_date = start_date	# keep track of current date
  ...
  def _storage_level_change(self, action: list(str) -> tuple(float,float):
	...
	new_stor_level = self.stor_lev + num_action
  ...
  def _update_stor_val(self, num_action):
  	...
  	"""
  	if num_action > 0.0:
  		self.stor_val = (self.stor_val * self.stor_lel + num_action * self.cur_price) / (self.stor_lev + num_action)
  ...
  def step(self, action):
	...
	# update observations
    	...
    	observations = [self.cur_date.year, self.cur_date.month, self.self.cur_price, self.stor_lev, self.stor_val]	
